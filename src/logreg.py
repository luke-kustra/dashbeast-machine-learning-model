"""Logistic Regression model for workout classification."""
...
DEFAULT_MODEL_PATH = os.path.join(ROOT_DIR, 'logreg_model.joblib')
DEFAULT_LABEL_MAP_PATH = os.path.join(ROOT_DIR, 'logreg_label_map.json')
...
def train(data_path: str = DEFAULT_DATA_PATH, label_map_path: str = DEFAULT_LABEL_MAP_PATH, model_path: str = DEFAULT_MODEL_PATH, val_split: float = 0.2, seed: int = 42) -> Dict[str, float]:
    """Train logistic regression model on workout data."""
...
def main():
    parser = argparse.ArgumentParser(description='Train or predict with logistic regression for workout classification')
    subparsers = parser.add_subparsers(dest='command', required=True)

    train_parser = subparsers.add_parser('train', help='Train logistic regression model')
    ...
    predict_parser = subparsers.add_parser('predict', help='Predict with trained logistic regression model')
